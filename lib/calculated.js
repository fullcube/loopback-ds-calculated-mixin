...
module.exports = function(Model, options) {
  ...
  Model.observe('before save', function(ctx, next) {
    ...
    if (!instance) {
      ...
      if (instanceId) {
        instance = ctx.data;
        instance = new ctx.Model(instance);
      } else {
    ...
    return Promise.map(Object.keys(options.properties), function(property) {
      ...
      var instanceCopy =  new ctx.Model(instance.toJSON());

      // Apply changes from partial update to make available for use in calculation.
      if (ctx.data) {
        const data = new ctx.Model(ctx.data).toJSON();
        instanceCopy = Object.assign(instanceCopy, data);
      }
